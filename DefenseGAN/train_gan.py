...
from ConvGanModel import ConvGenerator, ConvDiscriminator
...
class TrainConvGAN:
    ...
    def train(self, epochs=100):
        ...
        for epoch in range(epochs):
            ...
            #     self.test(block=False, show_time=10)
    ...
    def test(self, block=True, show_time=5):
        test_input = torch.randn(size=(16, self.latent_dim), device=self.device)
        gen_imgs = self.generator(test_input)
        draw_img_groups([gen_imgs], block=block, show_time=show_time)
